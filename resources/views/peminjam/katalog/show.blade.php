<x-app-layout>
    <x-slot name="pageTitle">Detail Buku</x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="md:flex">
                    ...
                    <div class="p-6 md:w-2/3">
                        ...
                        <span class="inline-block px-3 py-1 text-xs font-semibold bg-indigo-100 text-indigo-800 rounded-full mb-4">{{ $buku->genre->nama_genre }}</span>

                        <div class="grid grid-cols-2 gap-4 mb-4 text-sm text-gray-700 bg-gray-50 p-4 rounded-lg border border-gray-100">
                            <div>
                                <span class="block text-gray-500 text-xs font-semibold uppercase">Penulis</span>
                                <span>{{ $buku->penulis ?? '-' }}</span>
                            </div>
                            <div>
                                <span class="block text-gray-500 text-xs font-semibold uppercase">Penerbit</span>
                                <span>{{ $buku->penerbit ?? '-' }}</span>
                            </div>
                            <div>
                                <span class="block text-gray-500 text-xs font-semibold uppercase">Tahun Terbit</span>
                                <span>{{ $buku->tahun_terbit ?? '-' }}</span>
                            </div>
                            <div>
                                <span class="block text-gray-500 text-xs font-semibold uppercase">ISBN</span>
                                <span>{{ $buku->isbn ?? '-' }}</span>
                            </div>
                        </div>

                        <p class="text-gray-600 mb-4">{{ $buku->deskripsi ?? 'Tidak ada deskripsi.' }}</p>

                        <div class="mb-4">
                            <span class="text-lg font-semibold {{ $buku->stok > 0 ? 'text-green-600' : 'text-red-600' }}">
                                Stok Tersedia: {{ $buku->stok }}
                            </span>
                        </div>

                        @if($buku->stok > 0)
                            <form action="{{ route('peminjam.cart.add', $buku) }}" method="POST" class="flex items-center space-x-3">
                                @csrf
                        ...
                        @else
                            <div class="space-y-3">
                                <p class="text-sm text-gray-600">Stok sedang kosong. Anda bisa melakukan booking untuk buku ini.</p>
