<x-app-layout>
    <x-slot name="pageTitle">Genre</x-slot>

    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-gray-900 leading-tight">Data Genre Buku</h2>
                <p class="mt-1 text-sm text-gray-600">Kelola genre untuk klasifikasi buku</p>
            </div>
            <a href="{{ route('admin.genre.create') }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-200 text-white text-sm font-semibold rounded-lg shadow-sm hover:shadow-md transition-all duration-200">
                        ...
                        <thead class="bg-gradient-to-r from-gray-50 to-white">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    <a href="{{ route('admin.genre.index', ['sort_by' => 'id', 'sort_direction' => request('sort_by') == 'id' && request('sort_direction') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center hover:text-indigo-600 transition-colors">
                                        #
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    <a href="{{ route('admin.genre.index', ['sort_by' => 'nama_genre', 'sort_direction' => request('sort_by') == 'nama_genre' && request('sort_direction') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center hover:text-indigo-600 transition-colors">
                                        Genre
                                        @if(request('sort_by') == 'nama_genre')
                                            @if(request('sort_direction') == 'asc')
                                                <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>
                                            @else
                                                <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20"><path d="M15 10l-5 5-5-5h10z"/></svg>
                                ...
                                <th scope="col" class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah Buku</th>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-semibold text-gray-900">{{ $genre->nama_genre }}</div>
                                                <div class="text-xs text-gray-500">Genre #{{ $loop->iteration + ($genres->currentPage() - 1) * $genres->perPage() }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="inline-flex items-center">
                                            @if($genre->alat_count > 0)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            <form action="{{ route('admin.genre.destroy', $genre) }}" method="POST" class="inline" data-confirm="Yakin ingin menghapus genre ini? Buku dalam genre ini akan kehilangan kategorinya." data-confirm-title="Konfirmasi Hapus Genre" data-confirm-type="danger">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <h3 class="text-base font-semibold text-gray-900 mb-2">Belum Ada Genre</h3>
                                            <p class="text-sm text-gray-500 mb-6 max-w-sm">Mulai dengan membuat genre pertama untuk mengklasifikasikan buku-buku sekolah</p>
                                            <a href="{{ route('admin.genre.create') }}" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm transition-all">
                                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                                </svg>
