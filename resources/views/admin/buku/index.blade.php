<x-app-layout>
    <x-slot name="pageTitle">Data Buku</x-slot>

    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-gray-900 leading-tight">Data Buku</h2>
                <p class="mt-1 text-sm text-gray-600">Kelola inventaris buku sekolah</p>
            </div>
            <a href="{{ route('admin.buku.create') }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-200 text-white text-sm font-semibold rounded-lg shadow-sm hover:shadow-md transition-all duration-200">
                ...
                <div class="bg-gradient-to-r from-gray-50 to-white border-b border-gray-200 p-6">
                    <form method="GET" class="flex flex-col md:flex-row gap-3">
                        ...
                        @if(request('search') || request('genre_id'))
                            <a href="{{ route('admin.buku.index') }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-white hover:bg-gray-50 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg transition-all duration-200">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    <a href="{{ route('admin.buku.index', array_merge(request()->all(), ['sort_by' => 'id', 'sort_direction' => request('sort_by') == 'id' && request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center hover:text-indigo-600 transition-colors">
                                        #
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    <a href="{{ route('admin.buku.index', array_merge(request()->all(), ['sort_by' => 'judul', 'sort_direction' => request('sort_by') == 'judul' && request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center hover:text-indigo-600 transition-colors">
                                        Buku
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Genre</th>
                                <th scope="col" class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    <a href="{{ route('admin.buku.index', array_merge(request()->all(), ['sort_by' => 'stok', 'sort_direction' => request('sort_by') == 'stok' && request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}" class="inline-flex items-center hover:text-indigo-600 transition-colors">
                                        Stok
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($bukus as $buku)
                                <tr class="hover:bg-gray-50 transition-colors duration-150 {{ $buku->stok == 0 ? 'bg-red-50/30' : '' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                @if($buku->penulis || $buku->isbn)
                                                    <p class="text-xs text-gray-500 mt-1">
                                                        {{ $buku->penulis ?? 'Tanpa penulis' }} 
                                                        @if($buku->isbn) | ISBN: {{ $buku->isbn }} @endif
                                                    </p>
                                                @endif
                                                @if($buku->deskripsi)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('admin.buku.destroy', $buku) }}" method="POST" class="inline" data-confirm="Yakin ingin menghapus buku ini? Data peminjaman terkait akan terpengaruh." data-confirm-title="Konfirmasi Hapus Buku" data-confirm-type="danger">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm text-gray-500 mb-6 max-w-sm">
                                                ...
                                                @else
                                                    Mulai dengan menambahkan buku pertama ke inventaris sekolah.
                                                @endif
                                            </p>
                                            @if(request('search') || request('genre_id'))
                                            ...
                                            @else
                                                <a href="{{ route('admin.buku.create') }}" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm transition-all">
                                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                                    </svg>
                                                    Tambah Buku Pertama
